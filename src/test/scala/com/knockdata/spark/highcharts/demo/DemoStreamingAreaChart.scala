...
//    dataFrame.groupBy("year").count().writeStream.
    ...
    val chartHolder = highcharts.streamingChart(seriesHolder, zHolder, "mockChartParagraph", "append")
  ...
  @Test
  def demoBasicAreaComplete(): Unit = {
    ...
    val zHolder = new MockZeppelinContextHolder()
    val chartHolder = highcharts.streamingChart(seriesHolder, zHolder, "mockChartParagraph", "complete")
    val query = chartHolder.start
    query.processAllAvailable()
